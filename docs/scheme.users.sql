  ...
  INSERT INTO guest (id, guest_id) VALUES
      (2, 'GU000001'),
      (3, 'GU000002'),
      (4, 'GU000003'),
      (5, 'GU000004');
...
BEGIN
  ...
  SELECT COALESCE(MAX(guest_id), 'GU000001') INTO last_guest_id FROM guest;
  SET @next_guest_id = get_next_role_id(last_guest_id);
  INSERT INTO guest (id, guest_id) VALUES (user_id, @next_guest_id);
...
BEGIN
  DECLARE user_id INT;
  DECLARE last_agent_id CHAR(8);
  IF ( EXISTS (SELECT * FROM user WHERE username = _username) ) THEN
    SELECT "USERNAME_EXISTS" as message;
  INSERT INTO user (name, last_name, username, password, role_id) 
  VALUES (_name, _last_name, _username, _password, 3);
  SET user_id = LAST_INSERT_ID();
  SELECT COALESCE(MAX(agent_id), 'AG000001') INTO last_agent_id FROM agent; 
  SET @next_agent_id = get_next_role_id(last_agent_id);
  INSERT INTO agent (id, agent_id) VALUES (user_id, @next_agent_id);
  SELECT "SUCCESS" as message;
END //
